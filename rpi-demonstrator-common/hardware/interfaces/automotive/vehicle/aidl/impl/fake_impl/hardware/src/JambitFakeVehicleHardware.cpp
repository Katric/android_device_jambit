...
#define LOG_TAG "JambitFakeVehicleHardware"
...
// 0x21403000
// set ID to higher number, so vendor props are not overridden by it.
constexpr int32_t STARTING_VENDOR_CODE_PROPERTIES_FOR_TEST =
        ...
        0x4005 | toInt(testpropertyutils_impl::VehiclePropertyGroup::VENDOR) |
...
JambitFakeVehicleHardware* gJambitFakeVehicleHardware = nullptr;

void globalBatteryChangeHandler() {
    if (gJambitFakeVehicleHardware != nullptr) {
        gJambitFakeVehicleHardware->handleBatteryChange();
    }
}

void JambitFakeVehicleHardware::storePropInitialValue(const ConfigDeclaration& config) {
...
JambitFakeVehicleHardware::JambitFakeVehicleHardware()
    : JambitFakeVehicleHardware(DEFAULT_CONFIG_DIR, OVERRIDE_CONFIG_DIR, false) {}

JambitFakeVehicleHardware::JambitFakeVehicleHardware(std::string defaultConfigDir,
...
JambitFakeVehicleHardware::~JambitFakeVehicleHardware() {
    ...
    mGeneratorHub.reset();
    gJambitFakeVehicleHardware = nullptr;
...
std::unordered_map<int32_t, ConfigDeclaration> JambitFakeVehicleHardware::loadConfigDeclarations() {
...
void JambitFakeVehicleHardware::init() {
    ...
    gJambitFakeVehicleHardware = this;
...
void JambitFakeVehicleHardware::handleBatteryChange() {
    unsigned long interruptTime = millis();
    if (interruptTime - mLastInterruptTime > ROTARY_DEBOUNCE_TIME) {
        ...
        auto currentBatteryLevelResult =
                mServerSidePropStore->readValue(toInt(VehicleProperty::EV_BATTERY_LEVEL));

        if (batteryCapacityResult.ok() && currentBatteryLevelResult.ok()) {
            float batteryCapacity = batteryCapacityResult.value()->value.floatValues[0];
            float currentBatteryLevel = currentBatteryLevelResult.value()->value.floatValues[0];

            float newBatteryLevel;
            if (clk ^ dt) {
                newBatteryLevel =
                        std::min(currentBatteryLevel + 0.02f * batteryCapacity, batteryCapacity);
            } else {
            ...
            if (newBatteryLevel != currentBatteryLevel) {
                auto updatedValue = mValuePool->obtain(VehiclePropertyType::FLOAT);
                updatedValue->prop = toInt(VehicleProperty::EV_BATTERY_LEVEL);
                updatedValue->areaId = 0;
                updatedValue->timestamp = elapsedRealtimeNano();
                updatedValue->value.floatValues = {newBatteryLevel};
                ...
                if (writeResult.ok()) {
                    ...
                    bool isLowFuel = batteryPercentage < LOW_BATTERY_TRESHHOLD;

                    // read current FUEL_LEVEL_LOW value
                    auto currentFuelLowResult =
                            mServerSidePropStore->readValue(toInt(VehicleProperty::FUEL_LEVEL_LOW));
                    if (currentFuelLowResult.ok()) {
                        auto fuelWarningValue = mValuePool->obtain(VehiclePropertyType::BOOLEAN);
                        fuelWarningValue->prop = toInt(VehicleProperty::FUEL_LEVEL_LOW);
                        fuelWarningValue->areaId = 0;
                        fuelWarningValue->timestamp = elapsedRealtimeNano();
                        fuelWarningValue->value.int32Values = {isLowFuel ? 1 : 0};

                        auto fuelWarningWriteResult =
                                mServerSidePropStore->writeValue(std::move(fuelWarningValue));
                        if (fuelWarningWriteResult.ok()) {
                            ALOGD("Fuel low warning updated: %s", isLowFuel ? "true" : "false");
                        } else {
                            ALOGE("Failed to write fuel low warning: %s",
                                  fuelWarningWriteResult.error().message().c_str());
                        }
                    } else {
                        ALOGE("Failed to read current fuel low warning state");
                    }
                } else {
                    ALOGE("Failed to write new battery level: %s",
...
void JambitFakeVehicleHardware::setAmbientLightColorToBatteryLevel(float batteryPercentage, bool force, bool writeResult) {
    ...
    // > 10% = yellow
    // <= 10 = red
    // LED color only changes if VendorVehicleProperty::AMBIENT_LIGHT_MODE is set to AmbientLightMode::BATTERY_LEVEL
    ...
    if (ambientLightModeResult.ok()) {
        ...
        ALOGE("Failed to read ambient light mode");
    }
}
...
void JambitFakeVehicleHardware::setAmbientLightColor(std::vector<int32_t> rgbValues) {
    auto ambientLightModeResult =
        mServerSidePropStore->readValue(toInt(VendorVehicleProperty::AMBIENT_LIGHT_MODE));
...
std::vector<VehiclePropConfig> JambitFakeVehicleHardware::getAllPropertyConfigs() const {
...
VehiclePropValuePool::RecyclableType JambitFakeVehicleHardware::createApPowerStateReq(
...
VehiclePropValuePool::RecyclableType JambitFakeVehicleHardware::createAdasStateReq(int32_t propertyId,
                                                                             int32_t areaId,
                                                                             int32_t state) {
...
VhalResult<void> JambitFakeVehicleHardware::setApPowerStateReport(const VehiclePropValue& value) {
...
int JambitFakeVehicleHardware::getHvacTempNumIncrements(int requestedTemp, int minTemp, int maxTemp,
                                                  int increment) {
...
void JambitFakeVehicleHardware::updateHvacTemperatureValueSuggestionInput(
...
VhalResult<void> JambitFakeVehicleHardware::setHvacTemperatureValueSuggestion(
...
bool JambitFakeVehicleHardware::isHvacPropAndHvacNotAvailable(int32_t propId, int32_t areaId) const {
...
VhalResult<void> JambitFakeVehicleHardware::isAdasPropertyAvailable(int32_t adasStatePropertyId) const {
...
VhalResult<void> JambitFakeVehicleHardware::setUserHalProp(const VehiclePropValue& value) {
...
JambitFakeVehicleHardware::ValueResultType JambitFakeVehicleHardware::getUserHalProp(
...
JambitFakeVehicleHardware::ValueResultType JambitFakeVehicleHardware::maybeGetSpecialValue(
        const VehiclePropValue& value, bool* isSpecialValue) const {
    *isSpecialValue = false;
    int32_t propId = value.prop;
    ValueResultType result;

    //check this, maybe it overrides??
...
JambitFakeVehicleHardware::ValueResultType JambitFakeVehicleHardware::getEchoReverseBytes(
        const VehiclePropValue& value) const {
...
void JambitFakeVehicleHardware::sendHvacPropertiesCurrentValues(int32_t areaId) {
...
void JambitFakeVehicleHardware::sendAdasPropertiesState(int32_t propertyId, int32_t state) {
...
VhalResult<void> JambitFakeVehicleHardware::maybeSetSpecialValue(const VehiclePropValue& value,
                                                           bool* isSpecialValue) {
    *isSpecialValue = false;
    VehiclePropValuePool::RecyclableType updatedValue;
    int32_t propId = value.prop;

    // try disabling test properties instead of changing start value
    if (propId >= STARTING_VENDOR_CODE_PROPERTIES_FOR_TEST &&
        propId < ENDING_VENDOR_CODE_PROPERTIES_FOR_TEST) {
        *isSpecialValue = true;
        return {};
...
VhalResult<void> JambitFakeVehicleHardware::setValue(const VehiclePropValue& value) {
...
SetValueResult JambitFakeVehicleHardware::handleSetValueRequest(const SetValueRequest& request) {
...
StatusCode JambitFakeVehicleHardware::getValues(std::shared_ptr<const GetValuesCallback> callback,
...
GetValueResult JambitFakeVehicleHardware::handleGetValueRequest(const GetValueRequest& request) {
...
JambitFakeVehicleHardware::ValueResultType JambitFakeVehicleHardware::getValue(
        const VehiclePropValue& value) const {
    // In a real VHAL implementation, this will send the request to vehicle bus if not already
...
DumpResult JambitFakeVehicleHardware::dump(const std::vector<std::string>& options) {
    DumpResult result;
...
std::string JambitFakeVehicleHardware::genFakeDataHelp() {
    return R"(
...
std::string JambitFakeVehicleHardware::parseErrMsg(std::string fieldName, std::string value,
                                             std::string type) {
    return StringPrintf("failed to parse %s as %s: \"%s\"\n%s", fieldName.c_str(), type.c_str(),
                        value.c_str(), genFakeDataHelp().c_str());
}

void JambitFakeVehicleHardware::generateVendorConfigs(
...
std::string JambitFakeVehicleHardware::genFakeDataCommand(const std::vector<std::string>& options) {
    if (options.size() < 2) {
...
VehiclePropValue JambitFakeVehicleHardware::createHwInputKeyProp(VehicleHwKeyInputAction action,
...
VehiclePropValue JambitFakeVehicleHardware::createHwKeyInputV2Prop(int32_t area, int32_t targetDisplay,
...
VehiclePropValue JambitFakeVehicleHardware::createHwMotionInputProp(
...
void JambitFakeVehicleHardware::eventFromVehicleBus(const VehiclePropValue& value) {
    mServerSidePropStore->writeValue(mValuePool->obtain(value));
}

std::string JambitFakeVehicleHardware::dumpHelp() {
    return "Usage: \n\n"
...
std::string JambitFakeVehicleHardware::dumpAllProperties() {
...
std::string JambitFakeVehicleHardware::dumpOnePropertyByConfig(int rowNumber,
...
std::string JambitFakeVehicleHardware::dumpOnePropertyById(int32_t propId, int32_t areaId) {
    VehiclePropValue value = {
            .prop = propId,
            .areaId = areaId,
    };
...
std::string JambitFakeVehicleHardware::dumpListProperties() {
    auto configs = mServerSidePropStore->getAllConfigs();
    if (configs.size() == 0) {
...
Result<void> JambitFakeVehicleHardware::checkArgumentsSize(const std::vector<std::string>& options,
                                                     size_t minSize) {
...
std::string JambitFakeVehicleHardware::dumpSpecificProperty(const std::vector<std::string>& options) {
...
std::vector<std::string> JambitFakeVehicleHardware::getOptionValues(
...
Result<VehiclePropValue> JambitFakeVehicleHardware::parsePropOptions(
...
std::string JambitFakeVehicleHardware::dumpSetProperties(const std::vector<std::string>& options) {
...
std::string JambitFakeVehicleHardware::dumpGetPropertyWithArg(const std::vector<std::string>& options) {
    if (auto result = checkArgumentsSize(options, 3); !result.ok()) {
        return getErrorMsg(result);
    }
...
std::string JambitFakeVehicleHardware::dumpSaveProperty(const std::vector<std::string>& options) {
...
std::string JambitFakeVehicleHardware::dumpRestoreProperty(const std::vector<std::string>& options) {
...
std::string JambitFakeVehicleHardware::dumpInjectEvent(const std::vector<std::string>& options) {
    if (auto result = checkArgumentsSize(options, 3); !result.ok()) {
        return getErrorMsg(result);
    }
...
StatusCode JambitFakeVehicleHardware::checkHealth() {
    // Always return OK for checkHealth.
    return StatusCode::OK;
}

void JambitFakeVehicleHardware::registerOnPropertyChangeEvent(
...
void JambitFakeVehicleHardware::registerOnPropertySetErrorEvent(
        std::unique_ptr<const PropertySetErrorCallback> callback) {
    // In JambitFakeVehicleHardware, we will never use mOnPropertySetErrorCallback.
...
StatusCode JambitFakeVehicleHardware::updateSampleRate(int32_t propId, int32_t areaId, float sampleRate) {
...
void JambitFakeVehicleHardware::onValueChangeCallback(const VehiclePropValue& value) {
    if (mOnPropertyChangeCallback == nullptr) {
        return;
    }
...
void JambitFakeVehicleHardware::loadPropConfigsFromDir(
...
Result<float> JambitFakeVehicleHardware::safelyParseFloat(int index, const std::string& s) {
    float out;
    if (!ParseFloat(s, &out)) {
...
Result<std::vector<uint8_t>> JambitFakeVehicleHardware::parseHexString(const std::string& s) {
...
JambitFakeVehicleHardware::PendingRequestHandler<CallbackType, RequestType>::PendingRequestHandler(
        JambitFakeVehicleHardware* hardware)
    : mHardware(hardware) {
...
template <class CallbackType, class RequestType>
void JambitFakeVehicleHardware::PendingRequestHandler<CallbackType, RequestType>::addRequest(
...
template <class CallbackType, class RequestType>
void JambitFakeVehicleHardware::PendingRequestHandler<CallbackType, RequestType>::stop() {
...
template <>
void JambitFakeVehicleHardware::PendingRequestHandler<JambitFakeVehicleHardware::GetValuesCallback,
    ...
    for (const auto& rwc : mRequests.flush()) {
        ATRACE_BEGIN("JambitFakeVehicleHardware:handleGetValueRequest");
    ...
    for (const auto& [callback, results] : callbackToResults) {
        ATRACE_BEGIN("JambitFakeVehicleHardware:call get value result callback");
...
template <>
void JambitFakeVehicleHardware::PendingRequestHandler<JambitFakeVehicleHardware::SetValuesCallback,
                                                SetValueRequest>::handleRequestsOnce() {
    std::unordered_map<std::shared_ptr<const SetValuesCallback>, std::vector<SetValueResult>>
            callbackToResults;
    for (const auto& rwc : mRequests.flush()) {
        ATRACE_BEGIN("JambitFakeVehicleHardware:handleSetValueRequest");
        auto result = mHardware->handleSetValueRequest(rwc.request);
        ATRACE_END();
        callbackToResults[rwc.callback].push_back(std::move(result));
    }
    for (const auto& [callback, results] : callbackToResults) {
        ATRACE_BEGIN("JambitFakeVehicleHardware:call set value result callback");
        (*callback)(std::move(results));
        ATRACE_END();
    }
}
